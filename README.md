...
1. **⚡ Sorter & Organizer**: Automatically categorizes files into Year/Month (`2024/05`), Meaningful Title Groups (`june_pearl/`, `silent_eyes/`, `guru/`), File Type Category (`Images/JPG`), Extension (`PDF/PNG`), Alphabetical (`A-Z`), or File Size folders.
2. **🎯 Meaningful Title Grouping (Stopword-Aware) & Single Random Catch-All**: Extracts meaningful multi-word topic titles by stripping leading stopwords (`the`, `a`, `an`, `my`, `your`, etc.) and taking the title prefix before digits or date stamps (e.g. `_the_june_pearl_-12102022-0001.mp4` -> `june_pearl/`, `the_silent_eyes_145-07062022-0001.mp4` -> `silent_eyes/`, `my_document_2024.pdf` -> `document/`). All files lacking a clean word (pure hashes, UUIDs, numeric IDs, or gibberish like `323f9w8ehf8awjefi`, `hfqgifcbkj9`) route into **ONE SINGLE** shared folder called `_Random/` (or `Unsorted/`), preventing folder-per-hash directory pollution.
